...
fn execute_agent_command(command: String, args: Vec<String>) -> PyResult<String> {
    ...
    rt.block_on(async {
        ...
        if output.status.success() {
            ...
            Err(PyRuntimeError::new_err(format!("Execution failed ({}): {}", output.status, stderr)))
        }
    })
}
...
fn search_logs(plugin: String, query: String, from_date: String, to_date: String) -> PyResult<Vec<String>> {
    use std::fs::File;
    ...
    for entry in WalkDir::new(base_dir).into_iter().filter_map(|e| e.ok()) {
        ...
        if file_name.ends_with(".zst") {
            let decoder = zstd::Decoder::new(file).map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
