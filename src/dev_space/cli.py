...
def main(
    quiet: bool = typer.Option(
        False, "--quiet", "-q", help="Suppress stderr logs."
    ),
    verbose: bool = typer.Option(
        False, "--verbose", "-v", help="Full debug trace to stderr."
    ),
    format: str = typer.Option(
        config.core.default_format, "--format", help="Structured output format (json|jsonl|md)."
    ),
...
def bootstrap():
    ...
    # Import the rust core dynamically to test compilation
    from dev_space import core
    result = core.execute_agent_command("echo hello")
